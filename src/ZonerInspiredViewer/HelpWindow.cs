...
namespace ZonerInspiredViewer
{
    ...
    internal sealed class HelpWindow : Window
    {
        internal const string Shortcuts =
            ...
            + "Wheel previews show the current image plus three previous and next thumbnails in the current sort/filter order, wrapping at folder ends (small folders repeat). Click to open a preview; hover to keep the strip visible. It hides after two seconds. Configure > Images chooses bottom center, left center, right center, or turns previews off. Right-button + wheel still zooms.\r\n"
            ...
            + "The toolbar's Zoom lock keeps the zoom level across images, tabs, rotation and resizing. It highlights when enabled and is saved in sessions/backups. Manual zoom still changes the locked level; AI-resolution changes preserve apparent size. Fit width and Fit height center the image without distortion and can enlarge small images; drag to pan the overflowing dimension. Original size (1:1) shows current pixels at 100%. Normal Fit / 0 still avoids enlarging small images. Configure > Shortcuts can assign keys to these commands.\r\n"
